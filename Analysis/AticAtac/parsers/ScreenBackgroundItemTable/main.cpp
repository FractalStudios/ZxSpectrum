    ...
    if (argc == 2)
    {
        ...
        if (dwin32Error == ERROR_SUCCESS)
        {
            ...
            for (ScreenBackgroundItemMap::const_iterator itr = screenBackgroundItemMap.begin ();
                 ...
                 ++itr)
            {
                ::wprintf (L"Screen %03.03u, Address = 0x%04.04x, BackgroundItems = %s\n", 
                           itr->first,
                           itr->second.m_wAddress,
                           DescribeBackgroundItemAddressList (itr->second.m_backgroundItemAddressList).c_str ());
            } // Endfor.
