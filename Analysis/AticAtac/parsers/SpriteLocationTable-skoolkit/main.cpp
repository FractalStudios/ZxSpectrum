...
std::wstring    BuildSklText (const SpriteLocationList &backgroundList,
                              const AddressMap              &addressMap)
{
    std::wostringstream str;

    str << L"; Sprite Location Table" << std::endl
        << L";";
    ...
    str << L"; #TABLE(default)" << std::endl
        ...
        << L"; { $05         | Colour Attribute      | Colour attribute. }" << std::endl
        << L"; { $06         | Flags2                | ??? }" << std::endl
        << L"; { $07         | Flags3                | ??? }" << std::endl
        << L"; TABLE#" << std::endl;
         ...
         ++itr)
    {
        ...
        str << DescribeWORD (itr->m_wAddress)
            ...
            << DescribeBYTE (itr->m_byRoom) << L","
            << DescribeBYTE (itr->m_byUnknown1) << L","
            << DescribeBYTE (itr->m_byXPosition) << L","
            << DescribeBYTE (itr->m_byYPosition) << L","
            << DescribeBYTE (itr->m_byColour) << L","
            << DescribeBYTE (itr->m_byUnknown2) << L","
            << DescribeBYTE (itr->m_byUnknown3)
            << L" ; " << DescribeGraphic (itr->m_byGraphicType) 
            << L", Screen " << DescribeBYTE (itr->m_byRoom) 
            << L" (" << DescribeBYTE (itr->m_byYPosition) << "," << DescribeBYTE (itr->m_byXPosition) << L")";

            if (itr->m_byColour)
                str << L", " << DescribeColour (itr->m_byColour);
            str << L"." << std::endl;
    } // Endfor.
